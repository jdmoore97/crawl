    ...
    : file(), chunk(), compiled(), context(), first(-1), last(-1), error()
{
    ...
    ostringstream out;
    const int err = lua_dump(ls, dlua_compiled_chunk_writer, &out, lua_toboolean(ls, 2));
    if (err)
    {
...
int dlua_chunk::load(CLua &interp)
{
    ...
    if (err)
        return err;
    ostringstream out;
    err = lua_dump(interp, dlua_compiled_chunk_writer, &out, lua_toboolean(interp, 2));
    if (err)
    {
